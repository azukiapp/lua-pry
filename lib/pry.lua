...
Pry.expose = {
    ["Pry"] = true, ["pry"] = true, ["Pretty"] = true,
    ["p"] = true, ["ls"] = true, ["dir"] = true,
    ["slice"] = true
}

function Pry:new(params)
    local obj = {}
    params = params or {}
    setmetatable(obj, self)
    self.__index = self
...
function Pry:init(params)
    ...
    self.env.Pry = expose["Pry"] and Pry or nil
    self.env.pry = expose["pry"] and self or nil
...
-- this is mostly meant for the pry launcher/main
-- a separate Pry instance may need to do something different so wouldn't call this
function Pry:start()
    -- startup message
    if not self.disable_startup_message then
        self:output('PRY: ' .. _VERSION)
    ...
    -- to allow for configuration of the pry instance, require a module 'pry_instance'
    if not self.disable_instance_config then
        pcall(function()
            require 'pry_instance'
        end)
    end
    ...
    if self.file then
        ...
        self.savef:write('! pry ', concat(self.args,' '),'\n')
    end
...
function Pry:helpers()
    local e = self.env
    -- object aliases
    for k, v in pairs { i = self, I = Pry, P = Pretty, env = e, e = e } do
        e[k] = v
    end
...
function Pry:precision(len,prec,all)
...
function Pry:line_handler(handler)
    self.line_handler_fn = handler
end

function Pry:global_handler(handler)
    self.global_handler_fn = handler
end

function Pry:print_variables()
    for name,v in pairs(self.declared) do
        print(name,type(self.env[name]))
    end
end

Pry.v = Pry.print_variables
Pry.vars = Pry.print_variables

function Pry:get_input()
    local lines, i, input, chunk, err = {}, 1
    while true do
        input = linenoise((i == 1) and self.prompt or self.prompt2)
...
function Pry:eval_lua(line)
    ...
    -- is it an expression?
    local chunk, err = loadstring(format("pry:wrap(%s)", line), self.chunkname)
    if err then -- otherwise, a statement?
        chunk, err = loadstring(format("pry:wrap((function() %s end)())", line), self.chunkname)
    end
    if err then
        self:output(err)
        return
...
-- require a lib and inject it into pry namespace
function Pry:import(lib, dont_complain)
...
-- inject @tbl into the pry namespace
function Pry:inject(tbl, dont_complain, lib)
    lib = lib or '<unknown>'
    if type(tbl) == 'table' then
...
-- try to bring in any pry configuration files; don't complain if this is unsuccessful.
-- note that the only things accessible at this point are the Pry and Pretty classes,
-- which should be good enough to set/override defaults
-- (for configuring the default instance, use the module 'pry_instance' - see Pry:start())
pcall(function()
    require 'pry_system' -- system wide defaults
end)
pcall(function()
    require 'pry_global' -- user defaults
end)

-- Unix linenoise support, if linenoise.so is available...
local os = require 'os'
local ln
local path_separator  = package.config:sub(1,1)
local ln_history_file = os.getenv("HOME") .. path_separator .. ".lua-pry_history"

if not ln then
  pcall(function()
    ln = require 'linenoise'
    linenoise = ln.linenoise
    saveline = function(data)
      ln.historyadd(data)
      ln.historysave(ln_history_file)
    end
    ln.historyload(ln_history_file)
  end)
end
...
if arg then
    ...
    while i <= #arg do
        ...
        if opt == '-' then
            opt = v:sub(2,2)
            if opt == 'h' then
                quit(0,"pry [[ -h | -g | -H | -l <lib> | -L <lib> | -t | -T | -s | -i | -p | <file> ] ... ]")
            elseif opt == 'g' then
            ...
            elseif opt == 't' then
                params['file'] = parm_value(opt,v,"pry.log")
            elseif opt == 'T' then
                params['file'] = 'pry_'..os.date ('%y_%m_%d_%H_%M')..'.log'
            elseif opt == 's' then
                params['strict'] = false
            elseif opt == 'v' then
                params['verbose'] = true
...
-- create an Pry instance
local pry = Pry:new(params)

-- expose pry to global environment so any modules/files loaded can see it
g.pry = pry
